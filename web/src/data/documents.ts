...
export const publicDocs: PublicDoc[] = [
    ...
    },
    {
        ...
        descriptions: {
            ...
            JA: "ゲーム分析は、行動指標とデータ駆動の最適化を通じて、プレイヤーの行動に何が起こるかを理解するための強力な手法を確立している[7]。しかし、特定の構造構成が観察された行動パターンを生成する理由を完全に解釈するためには、分析には補完的な定性層、すなわちデータの背後にある因果メカニズムを説明する構造的文脈が必要である。本稿では、インタラクティブシステムにおける参加性向を分析するためのデザイン理論的監査フレームワークを提案する。エンゲージメントを単一の心理状態としてモデル化するのではなく、参加を短期的関与と長期的価値追求の間の緊張関係として特徴づけ、システムの価値スコープ（SV）によって調整されるものとする。このフレームワークは、決定空間を7つの分析的に分離可能な次元に分解し、結果のペイオフ、総コスト、時間的コミットメント、動機、失敗コスト、リスクなどを含む。この分解は形式的に最小または予測的であると主張するものではないが、埋没コストと失敗コストなど、しばしば混同されるメカニズムを区別し、マルチエージェント設定における体系的な増幅因子としてリスクの絡み合いを導入することによって、構造化されたデザイン監査をサポートするように構成されている。参加を純粋に経験的な応答としてではなく、合理的な監査プロセスとして捉えることにより、本研究は、経験的なリテンションチューニングを超えて、エンゲージメント失敗の構造的原因を診断するための分析的レンズをデザイナーや研究者に提供することを目指している。"
        },
        category: 'Paper',
