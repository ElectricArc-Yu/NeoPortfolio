import type { Translations } from './types';

export const translations: Translations = {
    EN: {
        'Commercial Projects': 'Commercial Projects',
        'Personal Projects': 'Personal Projects',
        'Project Info': 'Project Info',
        'Quick Links': 'Quick Links',
        'Project Introduce': 'Project Introduce',
        'Tech Stack': 'Tech Stack',
        'Work Distribution': 'Work Distribution',
        'Status': 'Status',
        'Professional Services': 'Professional Services',
        'View Details': 'View Details',
        'Experience': 'Experience',
        'Education': 'Education',
        'Skills': 'Skills',
        'Certifications': 'Certifications',
        'Platform': 'Platform',
        'Project Cycle': 'Project Cycle',
        'Team Size': 'Team Size',
        'Project Role': 'Project Role',
        'GDD Available': 'GDD Available',
        'Contact Me': 'Contact Me',
        'Media': 'Media',
        'Home': 'Home',
        'Resume': 'Resume',
        'Back to List': 'Back to List',
        'Back to Top': 'Back to Top',
        'Project Timeline': 'Project Timeline',
        'All': 'All',
        'Commercial': 'Commercial',
        'Personal': 'Personal',
        'Action': 'Action',
        'RPG': 'RPG',
        'Strategy': 'Strategy',
        'Puzzle': 'Puzzle',
        'Simulation': 'Simulation',
        'Survival': 'Survival',
        'Horror': 'Horror',
        'Platformer': 'Platformer',
        'Tool': 'Tool',
        'Analysis': 'Analysis',
        'Visual Novel': 'Visual Novel',
        'Action RPG': 'Action RPG',
        'FPS': 'FPS',
        'Hero Shooter': 'Hero Shooter',
        'World-building': 'World-building',
        'IP Framework': 'IP Framework',
        'SoC': 'SoC',
        'Other': 'Other',
        'No Image': 'No Image Available',
        'Portfolio': 'Portfolio',
        'Services': 'Services',
        'Paper Work': 'Writing & Lectures',
        'Personal Site': 'Personal Site',
        'GDD': 'Game Design',
        'Soon': 'Coming Soon',
        'Expected': 'Expected',
        'Back': 'Back',
        'Listen on': 'Listen on',
        'Stay Tuned': 'Stay Tuned',
        'Items': 'Items',
        'Eps': 'Eps',
        'Episodes': 'Episodes',
        'View Now': 'View Now',
        'Contact Subtitle': 'Feel free to reach out via email.',
        'Contact Immediately': 'Contact Now',
        'Lead Designer': 'Lead Designer',
        'Lead Programmer': 'Lead Programmer',
        'Designer': 'Designer',
        'Developer': 'Developer',
        'Founder': 'Founder',
        'Creative Director': 'Creative Director',
        'Game Designer': 'Game Designer',
        'System Designer': 'System Designer',
        'Combat Designer': 'Combat Designer',
        'Level Designer': 'Level Designer',
        'Solo Developer': 'Solo Developer',
        'Project Lead': 'Project Lead',
        'Full Stack Design': 'Full Stack Design',
        'Pre-production Management': 'Pre-production Management',
        'Narrative Design': 'Narrative Design',
        'IP Strategy': 'IP Strategy',
        'UX Design': 'UX Design',
        'UE5 Blueprint': 'UE5 Blueprint',
        'Lead Writer': 'Lead Writer',
        'Design': 'Design',
        'Programming': 'Programming',
        'Art': 'Art',
        'Blueprint': 'Blueprint',
        'Price': 'Price',
        'Community': 'Community',
        'Contact for Services': 'Contact for Services',
        'People': 'People',
        'Hours': 'Hours',
        'Pages': 'Pages',
        'Words': 'Words',
        'Audit': 'Audit',
        'Lecture': 'Lecture',
        'Paper': 'Paper',
        'Preprint': 'Preprint',
        'Internal': 'Internal',
        'Academic Papers': 'Academic Papers',
        'Lectures & Writing': 'Lectures & Writing',
        'Past Paper Work Examples': 'Past Paper Work Examples',
        'Published': 'Published',
        'Abstract': 'Abstract',
        'Collapse': 'Collapse',
        'Expand': 'Expand',
        'Target': 'Target',
        'Design Tests': 'Design Tests',
        'Analysis & Research': 'Analysis & Research',
        'Methodology & Tutorials': 'Methodology & Tutorials',
        'Full Game Design Documents': 'Full Game Design Documents',
        'Video': 'Video',
        'Article': 'Article',
        'Podcast': 'Podcast',
        'Chinese': 'Chinese',
        'English': 'English',
        'Miscellaneous': 'Miscellaneous',
        'Miscellaneous Description': 'Past project prototypes, code demos, and non-series content.',
        'Miscellaneous Content': 'Project showcases, prototype recordings, and independent analysis content.',
        'Scheduled': 'Scheduled',
        'Chinese Name': 'Yu Chenhaoran',
        'Present': 'Present',
        'Date Unknown': 'Date Unknown',
        'Collection': 'Collection',
        'View Certificate': 'View Certificate',
        'Verify Online': 'Verify Online',
        'Verified on': 'Verified on',
        'Design Tests Description': 'Design tests completed during job applications, demonstrating system design and problem-solving skills.',
        'Analysis & Research Description': 'In-depth analysis of published game content, exploring design philosophy and system architecture.',
        'Methodology & Tutorials Description': 'Design methodologies and professional development tutorials to enhance professional skills.',
        'Full Game Design Documents Description': 'Complete game design documents covering system design and level planning.',
        'Service Suspend Pattern': 'Service suspended from {start} to {end}.',
        'Service Start Pattern': 'Service officially starts from {date}.',
        'Export Resume': 'Export Resume',
        'Export Academic': 'Academic CV',
        'Export Academic Desc': 'Papers, Education, Experience, Timeline',
        'Export Professional': 'Professional Resume',
        'Export Professional Desc': 'Experience, Skills, Papers, Projects',
        'Export Full': 'Full Export',
        'Export Full Desc': 'All sections — complete overview',
        'Export Format Note': 'Exports as HTML · Print to PDF for best results',
        'Consultant': 'Consultant',
    },
    CN: {
        'Commercial Projects': '商业项目',
        'Personal Projects': '个人项目',
        'Project Info': '项目信息',
        'Quick Links': '快速链接',
        'Project Introduce': '项目简述',
        'Tech Stack': '技术栈',
        'Work Distribution': '工作量分布',
        'Type': '项目类型',
        'Sales': '销量 / 热度',
        'Work Hours': '工作时长',
        'Status': '项目状态',
        'View Details': '查看详情',
        'Experience': '工作经历',
        'Education': '教育背景',
        'Skills': '技能储备',
        'Certifications': '证书与资质',
        'Platform': '平台',
        'Project Cycle': '项目周期',
        'Team Size': '团队大小',
        'Project Role': '项目角色',
        'GDD Available': '包含设计文档',
        'Contact Me': '联系我',
        'Media': '自媒体',
        'Home': '首页',
        'Resume': '简历',
        'Back to List': '返回列表',
        'Back to Top': '回到顶部',
        'Project Timeline': '参与项目时间轴',
        'All': '全部',
        'Commercial': '商业',
        'Personal': '个人',
        'Action': '动作',
        'RPG': 'RPG',
        'Strategy': '策略',
        'Puzzle': '解谜',
        'Simulation': '模拟',
        'Survival': '生存',
        'Horror': '恐怖',
        'Platformer': '平台跳跃',
        'Tool': '工具',
        'Analysis': '分析',
        'Visual Novel': '视觉小说',
        'Action RPG': '动作RPG',
        'FPS': 'FPS',
        'Hero Shooter': '英雄射击',
        'World-building': '世界观构建',
        'IP Framework': 'IP 框架',
        'SoC': 'SoC',
        'Other': '其他',
        'No Image': '暂无图片',
        'Portfolio': '作品集',
        'Services': '有偿服务',
        'Paper Work': '写作与演讲',
        'Personal Site': '个人站',
        'GDD': '设计文档',
        'Soon': '即将发布',
        'Expected': '预计',
        'Back': '返回',
        'Listen on': '收听平台',
        'Stay Tuned': '关注频道',
        'Items': '项',
        'Eps': '集',
        'Episodes': '集',
        'View Now': '点击查看',
        'Contact Subtitle': '随时欢迎通过邮件与我联系。',
        'Contact Immediately': '立即联系',
        'Lead Designer': '主策划',
        'Lead Programmer': '主程序',
        'Designer': '策划',
        'Developer': '程序',
        'Founder': '创始人',
        'Creative Director': '创意总监',
        'Game Designer': '游戏策划',
        'System Designer': '系统策划',
        'Combat Designer': '战斗策划',
        'Level Designer': '关卡策划',
        'Solo Developer': '独立开发',
        'Project Lead': '项目负责人',
        'Full Stack Design': '全栈设计',
        'Pre-production Management': '前瞻管理',
        'Narrative Design': '叙事设计',
        'IP Strategy': 'IP 战略',
        'UX Design': 'UX 设计',
        'UE5 Blueprint': 'UE5 蓝图',
        'Lead Writer': '主笔',
        'Design': '设计',
        'Programming': '程序',
        'Art': '美术',
        'Blueprint': '蓝图',
        'Price': '价格',
        'Community': '工会',
        'Contact for Services': '咨询服务',
        'People': '人',
        'Hours': '小时',
        'Pages': '页',
        'Words': '字',
        'Audit': '审计',
        'Lecture': '讲座',
        'Paper': '论文',
        'Preprint': '预印本',
        'Internal': '内部',
        'Academic Papers': '学术论文',
        'Lectures & Writing': '演讲与著作',
        'Past Paper Work Examples': '过往文书案例',
        'Published': '发布页',
        'Abstract': '摘要',
        'Collapse': '收起',
        'Expand': '展开',
        'Target': '目标岗位',
        'Design Tests': '策划测试题',
        'Analysis & Research': '分析研究',
        'Methodology & Tutorials': '方法论与教程',
        'Full Game Design Documents': '完整设计文档',
        'Video': '视频',
        'Article': '文章',
        'Podcast': '播客',
        'Chinese': '中文',
        'English': '英文',
        'Miscellaneous': '杂项',
        'Miscellaneous Description': '过往项目原型、代码演示以及非系列内容。',
        'Miscellaneous Content': '项目演示视频、早期原型记录以及独立的分析内容。',
        'Scheduled': '预计',
        'Chinese Name': '于陈浩然',
        'Present': '至今',
        'Date Unknown': '时间未知',
        'Collection': '合集',
        'View Certificate': '查看证书',
        'Verify Online': '在线验证',
        'Verified on': '认证于 ',
        'Design Tests Description': '求职过程中完成的策划岗位测试题，展示系统设计与问题解决能力。',
        'Analysis & Research Description': '对已发布游戏内容的深度分析，探索设计理念与系统架构。',
        'Methodology & Tutorials Description': '设计方法论与职业发展教程，帮助提升专业能力。',
        'Full Game Design Documents Description': '完整的游戏设计文档，涵盖系统设计与关卡规划。',
        'Service Suspend Pattern': '将在 {start} 至 {end} 暂停服务业务。',
        'Service Start Pattern': '服务从 {date} 后正式开始。',
        'Export Resume': '导出简历',
        'Export Academic': '学术简历',
        'Export Academic Desc': '论文、学历、工作履历、项目时间轴',
        'Export Professional': '职业简历',
        'Export Professional Desc': '工作履历、技能、论文、商业项目',
        'Export Full': '全部导出',
        'Export Full Desc': '所有章节 — 完整概览',
        'Export Format Note': '导出为 HTML · 打印为 PDF 效果最佳',
        'Consultant': '顾问',
    },
    JA: {
        'Commercial Projects': '商業プロジェクト',
        'Personal Projects': '個人プロジェクト',
        'Project Info': 'プロジェクト情報',
        'Quick Links': 'クイックリンク',
        'Project Introduce': 'プロジェクト紹介',
        'Tech Stack': '技術スタック',
        'Work Distribution': '作業配分',
        'Type': 'ゲームタイプ',
        'Sales': '売上 / ヒット',
        'Work Hours': '制作時間',
        'Status': 'ステータス',
        'View Details': '詳細を見る',
        'Experience': '職歴',
        'Education': '学歴',
        'Skills': 'スキル',
        'Certifications': '認定・資格',
        'Platform': 'プラットフォーム',
        'Project Cycle': '制作期間',
        'Team Size': 'チーム規模',
        'Project Role': '担当役割',
        'GDD Available': '企画書あり',
        'Contact Me': 'お問い合わせ',
        'Media': 'メディア',
        'Home': 'ホーム',
        'Resume': '履歴書',
        'Back to List': 'リストに戻る',
        'Back to Top': 'トップに戻る',
        'Project Timeline': 'プロジェクト年表',
        'All': 'すべて',
        'Commercial': '商業',
        'Personal': '個人',
        'Action': 'アクション',
        'RPG': 'RPG',
        'Strategy': 'ストラテジー',
        'Puzzle': 'パズル',
        'Simulation': 'シミュレーション',
        'Survival': 'サバイバル',
        'Horror': 'ホラー',
        'Platformer': 'プラットフォーマー',
        'Tool': 'ツール',
        'Analysis': '分析',
        'Visual Novel': 'ビジュアルノベル',
        'Action RPG': 'アクションRPG',
        'FPS': 'FPS',
        'Hero Shooter': 'ヒーローシューター',
        'World-building': '世界観構築',
        'IP Framework': 'IP枠組み',
        'SoC': 'SoC',
        'Other': 'その他',
        'No Image': '画像なし',
        'Portfolio': 'ポートフォリオ',
        'Services': '有償サービス',
        'Paper Work': '執筆と講演',
        'Personal Site': '公式サイト',
        'GDD': '設計書',
        'Soon': '近日公開',
        'Expected': '予定',
        'Back': '戻る',
        'Listen on': '配信プラットフォーム',
        'Stay Tuned': 'チャンネル登録',
        'Items': '個',
        'Eps': '話',
        'Episodes': '話',
        'View Now': '今すぐ見る',
        'Contact Subtitle': 'お気軽にメールでお問い合わせください。',
        'Contact Immediately': '今すぐ連絡',
        'Lead Designer': 'リードプランナー',
        'Lead Programmer': 'メインプログラマー',
        'Designer': 'プランナー',
        'Developer': 'デベロッパー',
        'Founder': '創設者',
        'Creative Director': 'クリエイティブディレクター',
        'Game Designer': 'ゲームプランナー',
        'System Designer': 'システムプランナー',
        'Combat Designer': 'コンバットプランナー',
        'Level Designer': 'レベルプランナー',
        'Solo Developer': '個人開発',
        'Project Lead': 'プロジェクト長',
        'Full Stack Design': 'フルスタックデザイン',
        'Pre-production Management': 'プリプロダクション管理',
        'Narrative Design': 'ナラティブデザイン',
        'IP Strategy': 'IP戦略',
        'UX Design': 'UXデザイン',
        'UE5 Blueprint': 'UE5ブループリント',
        'Lead Writer': 'リードライター',
        'Design': 'デザイン',
        'Programming': 'プログラミング',
        'Art': 'アート',
        'Blueprint': 'ブループリント',
        'Price': '価格',
        'Community': 'コミュニティ',
        'Contact for Services': 'サービスのお問い合わせ',
        'People': '人',
        'Hours': '時間',
        'Pages': 'ページ',
        'Words': '字',
        'Audit': '監査',
        'Lecture': '講義',
        'Paper': '論文',
        'Preprint': 'プレプリント',
        'Internal': '内部',
        'Academic Papers': '学術論文',
        'Lectures & Writing': '講演と著書',
        'Past Paper Work Examples': '過去の書類作成事例',
        'Published': '掲載ページ',
        'Abstract': '要約',
        'Collapse': '閉じる',
        'Expand': '展開',
        'Target': '志望職種',
        'Design Tests': '企画テスト',
        'Analysis & Research': '分析・研究',
        'Methodology & Tutorials': 'メソッド・チュートリアル',
        'Full Game Design Documents': 'ゲーム企画書',
        'Video': '動画',
        'Article': '記事',
        'Podcast': 'ポッドキャスト',
        'Chinese': '中国語',
        'English': '英語',
        'Miscellaneous': 'その他',
        'Miscellaneous Description': '過去のプロジェクトプロトタイプ、コードデモ、および非シリーズコンテンツ。',
        'Miscellaneous Content': 'プロジェクト紹介動画、初期プロトタイプの記録、および独立した分析コンテンツ。',
        'Scheduled': '予定',
        'Chinese Name': '于陳浩然',
        'Present': '現在',
        'Date Unknown': '期間不明',
        'Collection': 'コレクション',
        'View Certificate': '証書を表示',
        'Verify Online': 'オンライン検証',
        'Verified on': '認定日 ',
        'Design Tests Description': '採用選考中に作成された企画職用テスト。システム設計と問題解決能力を示しています。',
        'Analysis & Research Description': 'リリース済みゲームタイトルの詳細な分析。デザイン哲学とシステム構成を掘り下げます。',
        'Methodology & Tutorials Description': 'デザイン手法とキャリア開発のチュートリアル。専門能力の向上を目的としています。',
        'Full Game Design Documents Description': 'システム設計からレベルプランニングまで網羅した完全なゲーム企画書。',
        'Service Suspend Pattern': '{start} から {end} までサービスを一時停止いたします。',
        'Service Start Pattern': 'サービスは {date} より正式に開始されます。',
        'Export Resume': '履歴書をエクスポート',
        'Export Academic': '学術CV',
        'Export Academic Desc': '論文、学歴、職歴、プロジェクト年表',
        'Export Professional': '職務経歴書',
        'Export Professional Desc': '職歴、スキル、論文、商業プロジェクト',
        'Export Full': '全データエクスポート',
        'Export Full Desc': '全セクション — 完全な概要',
        'Export Format Note': 'HTML形式でエクスポート · PDF印刷推奨',
        'Consultant': 'コンサルタント',
    }
};
